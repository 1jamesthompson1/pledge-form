...
export async function sendPledgeNotification(pledge, pdfBuffer, versionInfo = {}) {
  ...
  const {
    formVersion, backendFormVersion, mismatch, suspect, suspectReason,
  } = versionInfo;
  const warnings = [
    ...(mismatch
      ? [
    ...
    ...(suspect
      ? [
          '*** POSSIBLE SPAM / FAST SUBMISSION ***',
          `This submission may not be genuine (${suspectReason || 'heuristic flagged'}).`,
          'It has still been emailed so a real pledge is never silently lost — please verify before acting on it.',
        ]
      : []),
  ];
  ...
  const message = {
    subject: `${isDev ? '[TEST] ' : ''}${mismatch ? '[VERSION MISMATCH] ' : ''}${suspect ? '[POSSIBLE SPAM] ' : ''}New pledge submission from ${pledge.parentName}`,
    body: {
      contentType: 'Text',
